...
public class Writer extends Thread {
  ...
  public void run () {
    for (int I = 0;  I < 5; I++) {
      ...
      try{
      	Synch.y.acquire(); //Mutual exclusion for manipulating writecount
      }
      catch(Exception e){}
      Synch.writecount++;	//incrementing number of writers
      if (Synch.writecount==1){
			try{
				 Synch.read.acquire();  // gain exclusive access, no readers can read, might have to wait for 
			}							// active readers to finish
			catch(Exception e){}
       }
      Synch.y.release(); //release mutual exclusion of writecount
      try{
        	Synch.wrt.acquire();	//exclusive access, no other writers can write
        }
        catch(Exception e){}
      ...
      Synch.wrt.release();	//done writing, signals other writers to proceed
      try{
        	Synch.y.acquire(); //mutual exclusion for manipulating writecount
        }
        catch(Exception e){}
        Synch.writecount--; //decrementing number of writers 
        if (Synch.writecount==0){
  			try{
  				 Synch.read.release(); //if no writers are waiting, then readers can read.
  			}
  			catch(Exception e){}
         }
        Synch.y.release(); //release mutual exclusion for manipulating writecount

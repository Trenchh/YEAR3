...
public class rwMonitor {
  ...
  private int waitingWriters; //holds number of writers waiting to write
  private int waitingReaders; //holds number of readers waiting to read

 private boolean canWrite; //used as condition for priority
  ...
  public synchronized void startRead () {
	  if(writecount > 0 || waitingWriters > 0) { // a reader can enter if no writers are waiting or writing, allows for many readers at once
		  waitingReaders++; //reader waiting
		  while(writecount > 0) {
			  try {
					wait(); //acquiring mutex
				} catch (InterruptedException e) {
				}
		  }
		  waitingReaders--; //reader no longer waiting
	  }
    readcount++; //reader reading
  ...
  public synchronized void endRead () {
    readcount--; //not reading anymore
	  if (readcount == 0) { //if last reader then writer can write
        canWrite = true; 
    } else {
    	canWrite  = false; //lets other readers in
    }
	 if (canWrite) {
		 notifyAll(); //writers can write
	 }
  ...
  public synchronized void startWrite () {
	  if(writecount > 0 || readcount > 0) { //writer enters if no other active writers and no waiting readers
		  waitingWriters++; //writer waiting
		  canWrite = false; //currently busy/cannot write
		  while(!canWrite) {
			  try {
					wait(); //acquire lock
				} catch (InterruptedException e) {
		  ...
		  waitingWriters--; //no longer waiting
	  }
	  canWrite = false; //no other writers can write
      writecount++; //writer writing
  ...
  public synchronized void endWrite () {
    writecount--; //writer no longer writing
    if(waitingReaders > 0) { // continues writing if no readers waiting
    	canWrite = false;
    } else {
    	canWrite = true;
    }
    notifyAll(); //wakes up anything waiting
